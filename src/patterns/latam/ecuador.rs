...
pub fn mask_ec_cedula_consistent(s: &str, hasher: &crate::patterns::consistent::ConsistentHasher, claims: &crate::patterns::TokenClaims) -> String {
    crate::patterns::mask_family(&EC_CEDULA_RE, s, claims, &|t, _, _| valid_cedula(t), &|d| hasher.encrypt(d).ok())
}

pub fn mask_ec_cedula_fpe(s: &str, cipher: &crate::patterns::fpe::FpeCipher, claims: &crate::patterns::TokenClaims) -> String {
    crate::patterns::mask_family(&EC_CEDULA_RE, s, claims, &|t, _, _| valid_cedula(t), &|d| cipher.encrypt(d).ok())
}
